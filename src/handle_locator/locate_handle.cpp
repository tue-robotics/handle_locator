...
void executeCB(const tue_msgs::LocateDoorHandleGoalConstPtr& goal) {
    // roughly crop the pc, cluster the outliers of the main plane and select the most likely cluster
    ros::NodeHandle n;
    ...
    if(listener_->waitForTransform(msg->header.frame_id, goal->handle_location_estimate.header.frame_id,
                                   msg->header.stamp, ros::Duration(1.0)))
    {
        try
        {
            listener_->transformPoint(msg->header.frame_id, goal->handle_location_estimate, original_handle_location);
        }
    ...
    // Crop the pointcloud based on the frame points, the y direction of the camera frame is the height
